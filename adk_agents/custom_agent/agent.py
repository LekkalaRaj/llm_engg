...
class StoryFlowAgent(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        # Ensure topic exists in session state
        if "topic" not in ctx.session.state:
            logger.info(f"[{self.name}] 'topic' not found in session state. Using default.")
            ctx.session.state["topic"] = "a brave kitten exploring a haunted house"
